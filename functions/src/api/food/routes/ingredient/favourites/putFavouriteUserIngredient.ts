 ...
 */
export async function putFavouriteUserIngredient(request: Request, response: Response): Promise<Response> {
  const userId = request.user.userId;
  const ingredientId = request.params.ingredientId;

  return favourites.addFavouriteUserIngredient(userId, ingredientId)
      .then(() => {
